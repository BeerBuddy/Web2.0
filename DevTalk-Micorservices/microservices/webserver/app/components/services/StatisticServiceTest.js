  ...
  describe("tests the StatisticService", function() {
    ...
    describe('StatisticService.getAccessStatistics', function() {
      it('should return an Object that can be passed to the Chart-directive',
         function() {
           service.getAccessStatistics().success(function(data, status, headers,
                                                          config) {
             var testObject = data;
    ...
    describe('StatisticService.getRegistrationData', function() {
      it('should return an Object that can be passed to the Chart-directive',
         function() {
           service.getRegistrationData().success(function(data, status, headers,
                                                          config) {
             var testObject = data;
             expect(testObject).toBeDefined();
             expect(testObject.labels.length).toBe(7);
      ...
      it('should return an object where the registrations should be higher or equal to the participations',
         function() {
           service.getRegistrationData().success(function(data, status, headers,
                                                          config) {
             var testObject = data;
             ...
             for (var p = 0; p < registrations.length; p++) {
               ...
               expect(participations[p])
                   .toBeGreaterThan(registrations[p] * 0.59);
             }
           });
         });
    });
